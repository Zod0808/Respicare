...
**Alcance**: consolida las 13 iteraciones del proyecto completo (Sprint 0 a Sprint 12)
...
**Incluye**: las 13 iteraciones documentadas — Sprint 0 (arquitectura y setup), Sprints 1-4 (MVP: autenticación, frontend/dashboard, servicios de IA, chatbot y analytics básicos), Sprints 5-8 (sistema de Machine Learning con Random Forest/XGBoost, explicabilidad SHAP e integración con el chatbot), Sprint 9 (refinamiento, documentación y preparación para producción) y Sprints 10-12 (interoperabilidad HL7/FHIR, dashboard ejecutivo y modelos ML predictivos).

**Excluye**: iteraciones futuras no documentadas aún en la metodología ágil del proyecto, como la integración completa de wearables con los servicios de IA (actualmente Health Connect/HealthKit son stubs y el pipeline de IA no consume datos de wearables) y el módulo de emergencias y ambulancias, previstos como iteraciones posteriores.

## 4. Entregables Esperados

Entregables verificables comprometidos, uno por cada una de las 13 iteraciones del proyecto:
...
Cronograma consolidado de las 13 iteraciones, expresado en semanas de proyecto (26 semanas / 6 meses en total), conforme a las duraciones documentadas por fase en `METODOLOGIA_AGIL_PROYECTO.md`.
...
| Wearables (Health Connect/HealthKit) permanecen como stubs sin integrarse al pipeline de IA | Medio | Documentado explícitamente como fuera de alcance de las 13 iteraciones actuales; planificado como iteración futura |
...
Métricas finales consolidadas del proyecto completo (13 iteraciones), tomadas de `METODOLOGIA_AGIL_PROYECTO.md`:
...
Las 13 iteraciones documentadas demuestran una ejecución ágil exitosa del proyecto completo, superando las metas planificadas de sprints (120%), story points (116%) y calidad (accuracy ML 105%, cobertura de pruebas 103-111%). Los desafíos superados incluyen la adaptación a TypeScript strict mode, la integración SHAP compleja y el despliegue multi-servicio con Docker. Las iteraciones futuras (no incluidas en este consolidado) deberán abordar la integración completa de wearables con el pipeline de IA y el módulo de emergencias y ambulancias.
